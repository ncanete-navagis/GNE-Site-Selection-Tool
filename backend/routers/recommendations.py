...
def generate_recommendation(
    ...
    """
    res = rec_service.create_recommendation(
        session=db,
...
def get_user_recommendations(
    ...
    if current_user.user_id != user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to access this user's recommendations")
    res = rec_service.get_user_recommendations(session=db, user_id=user_id)
    return res
...
def get_recommendation(
    ...
    """
    res = rec_service.get_recommendation(session=db, rlocation_id=rlocation_id)
    return res
