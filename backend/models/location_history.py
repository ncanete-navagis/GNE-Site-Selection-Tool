"""
models/location_history.py — Location History ORM model.

PK is hlocation_id (UUID).
FKs: user_id → users.id (Integer — NOT string), analysis_id → analyses.analysis_id (UUID).
barangay_pcode is plain Text — no FK constraint (barangay data lives in ph_barangays
which uses ADM4_PCODE; we store the code as a denormalised text field for simplicity).
...
class LocationHistory(Base):
    ...
    hlocation_id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, nullable=False)
    ...
    analysis = relationship("Analysis", back_populates="location_histories")
